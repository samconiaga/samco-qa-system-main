...
export default function Index({ title }) {
    ...
    const loadTableData = () => {
        setIsLoading(true);
        axios.get(route('datatable.change-requests'), {
            params: {
                page: currentPage,
    ...
    return (
        <AppLayout>
            <Breadcrumb title={t('change_requests')} subtitle={`${t('change_requests')}`} />
            <div className="container">
                <div className="d-flex justify-content-end mb-3">
                    {permissions.includes('Create Change Control') && (
                        <Link href={route('change-requests.create')} className="btn btn-sm btn-danger" data-title={t('create_change_request')}>
                                    ...
                                    <table className="table table-freeze table-bordered" style={{ minWidth: "1500px" }}>
                                        <thead className="table-light"
                                            ...
                                            <tr>
                                                ...
                                                <th style={{ width: "300px" }}>{t("request_number")}</th>
                                                <th style={{ width: "200px" }}>{t("title")}</th>
                                                <th style={{ width: "200px" }}>{t("requested_date")}</th>
                                                <th style={{ width: "150px" }}>{t("initiator_name")}</th>
                                                <th style={{ width: "200px" }}>{t("email")}</th>
                                                <th style={{ width: "200px" }}>{t("department")}</th>
                                            ...
                                            {isLoading ? (
                                                ...
                                                tableData.map((row, i) => (
                                                    <tr key={row.id}>
                                                        ...
                                                        <td>{row?.title}</td>
                                                        <td className="text-center">{toDateString(row?.requested_date)}</td>
                                                        <td>{row?.initiator_name}</td>
                                                        <td>{row?.employee?.user?.email}</td>
                                                        <td >{row?.department?.name}</td>
                                                        <td className="text-center">
                                                            {
                                                                (row.overall_status ?? '-') === 'Pending' ? (
                                                                    <span className="badge bg-warning">{t('waiting_initiator_manager_approval')}</span>
                                                                ) : (row.overall_status ?? '-') === 'In Progress' ? (
                                                                    <span className="badge bg-warning text-dark">{t('in_progress')}</span>
                                                                ...
                                                                ) : (row.overall_status ?? '-') === 'Reviewed' ? (
                                                                    <span className="badge bg-info">{t('review_by_relevant_departments_and_prodev')}</span>
                                                                ) : (row.overall_status ?? '-') === 'Waiting SPV Approval' ? (
                                                                    <span className="badge bg-warning">{t('waiting_spv_approval')}</span>
                                                                ) : (row.overall_status ?? '-') === 'Waiting QA Manager Approval' ? (
                                                                    <span className="badge bg-warning">{t('waiting_qa_manager_approval')}</span>
                                                                ) : (row.overall_status ?? '-') === 'Waiting Plant Manager Approval' ? (
                                                                    <span className="badge bg-warning">{t('waiting_plant_manager_approval')}</span>
                                                                ) : (row.overall_status ?? '-') === 'Closed' ? (
                                                                    <span className="badge bg-danger">{t('Closed')}</span>
                                                                ) : (
                                                                    <span className="badge bg-secondary">{row.overall_status ?? '-'}</span>
                                                                )
                                                            }
                                                        </td>
                                                        <td>
                                                            {[
                                                                ...
                                                                ['Rejected', 'Draft'].includes(row.overall_status) &&
                                                                permissions.includes('Create Change Control') &&
                                                                ...
                                                                (row.overall_status !== 'Draft' || row.overall_status !== 'Rejected') &&
                                                                // Always show button
