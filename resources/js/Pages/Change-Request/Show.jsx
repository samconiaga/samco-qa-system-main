...
export default function Show({ title, changeRequest, ...props }) {
    ...
    const canViewApproveRejectButton =
        ...
        changeRequest.approvals?.some(
            ({ decision, approver_id }) =>
                decision === "Pending" && approver_id === user?.employee?.id,
        );

    const canCloseButton =
        status === "Waiting Close" &&
        ((permissions.includes("Approve QA Manager") &&
            changeRequest?.closing?.qa_manager_sign == null) ||
            (permissions.includes("Approve QA SPV") &&
                changeRequest?.closing?.qa_spv_sign == null));
    const handleClose = () => {
        setModalShow(false);
        clearErrors();
        setData({
    ...
    const handleCloseRequest = () => {
        if (permissions.includes("Approve QA SPV")) {
            confirmAlert(
                t("are_you_sure"),
                t("close_request_description"),
                "warning",
                () => {
                    return router.visit(
                        route("change-requests.qa-approval", changeRequest.id),
                    );
                },
            );
        } else {
    ...
    const handlePrint = (id) => {
        window.open(route("change-requests.print", id), "_blank");
    };
    return (
        <AppLayout>
            <Breadcrumb
                title={t("change_request_detail")}
                    ...
                    <div className="user-grid-card position-relative border radius-16 overflow-hidden bg-base h-100">
                        <div className="pb-24 ms-16 mb-24 me-16  mt--100">
                            <div className="text-center border-top-0 border-start-0 border-end-0 mt-50">
                                ...
                                <div className="mt-20">
                                    {permissions.includes(
                                        "Review Prodev Manager",
                                    ) &&
                                        !changeRequest.regulatory && (
                                            <Link
                                                href={route(
                                                    "change-requests.regulatory-assessments.index",
                                                    { id: changeRequest.id },
                                                )}
                            ...
                            <div className="mt-24">
                                ...
                                >
                                    <Tab
                                        eventKey="change-initiation"
                                        title={t("change_initiation")}
                                    >
                                        <ChangeInitiationDetail
                                            changeRequest={changeRequest}
                                        />
                                    </Tab>

                                    <Tab
                                        eventKey="impact-risk-assesment"
                                        title={t("impact_risk_assesment")}
                                    >
                                        <ImpactRiskAssessmentDetail
                                            impactRiskAssessment={
                                                changeRequest.impact_risk_assesment
                                            }
                                            data={data}
                                            setData={setData}
                                            errors={errors}
                                    ...
                                    <Tab
                                        eventKey="affected-products"
                                        title={t("affected_products")}
                                    >
                                        <AffectedProduct
                                            changeRequest={changeRequest}
                                        />
                                    ...
                                    <Tab
                                        eventKey="regulatory-assessments"
                                        title={t("regulatory_assessments")}
                                    >
                                        <RegulatoryAssessmentDetail
                                            changeRequest={changeRequest}
                                        />
                                    </Tab>
                                    ...
                                    <Tab
                                        eventKey="qa-risk-assesment"
                                        title={t("risk_mitigation")}
                                    >
                                        <QaRiskAssessmentDetail
                                            riskAssessment={
                                                changeRequest.qa_risk_assesment
                                            }
                                        />
                                    </Tab>
                                </Tabs>
            ...
            <Modal show={modalShow} onHide={handleClose}>
                <Modal.Header closeButton>
                    <Modal.Title>
                        <h6>
                            {t("enter_attribute", {
                                attribute: t("comment_or_evaluation"),
                            })}
                        </h6>
                    </Modal.Title>
                </Modal.Header>
