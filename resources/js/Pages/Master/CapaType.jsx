...
export default function CapaType() {
    ...
    const loadTableData = () => {
        setIsLoading(true);
        axios.get(route('datatable.capa-types'), {
            params: {
                page: currentPage,
    ...
    const handleDelete = (id) => {
        confirmAlert(t('are_you_sure'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.capa-types.destroy', id), {
                onSuccess: (page) => {
    ...
    const handleEdit = (id) => {
        setEditId(id);
        setModalTitle(t('edit_capa_type'));
        setShow(true);
        clearErrors();
        reset();
        const capaType = tableData.find(ct => ct.id === id);
        if (capaType) {
            setData({
                name: capaType.name,
            });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (editId) {
            return put(route('master-data.capa-types.update', editId), {
                onSuccess: (page) => {
                    setData({
                        name: '',
                    });
        ...
        post(route('master-data.capa-types.store'), {
            onSuccess: (page) => {
            ...
            <AppLayout>
                ...
                <div className="container">
                    <div className="d-flex justify-content-end mb-3">
                        <Button type="button" className="btn btn-sm btn-danger me-3" data-title={t('add_new_capa_type')} onClick={handleShow}>
                            <Icon icon="line-md:plus" className="me-2" width="20" height="20" />
                            {t('add_new_capa_type')}
                        </Button>
                        <Link href={route('master-data.capa-types.trash')} aria-label={t('trash')} className="btn btn-sm btn-secondary" >
                ...
                <Modal show={show} onHide={handleClose}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <Modal.Body>
                            <div className="mb-3">
                                <label htmlFor="name" className="form-label">{t('capa_type_name')}</label>
                                <TextInput name="name"
                                    className="form-control"
                                    autoComplete="off"
                                    onChange={(e) => setData('name', e.target.value)}
                                    placeholder={t('enter_attribute', { 'attribute': t('capa_type_name') })}
                                    value={data.name}
