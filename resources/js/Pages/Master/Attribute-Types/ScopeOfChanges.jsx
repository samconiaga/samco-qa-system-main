...
export default function Departments() {
    ...
    const loadTableData = () => {
        setIsLoading(true);
        axios.get(route('datatable.scope-of-changes'), {
            params: {
                page: currentPage,
    ...
    const handleDelete = (id) => {
        confirmAlert(t('are_you_sure'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.attribute-types.scope-of-changes.destroy', id), {
                onSuccess: (page) => {
    ...
    const handleEdit = (id) => {
        setEditId(id);
        setModalTitle(t('edit_scope_of_change'));
        setShow(true);
        clearErrors();
        reset();
        const datas = tableData.find(dat => dat.id === id);
        if (datas) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (editId) {
            return put(route('master-data.attribute-types.scope-of-changes.update', editId), {
                onSuccess: (page) => {
                    setData('name', '');
                    setEditId(null);
                    setShow(false);
        ...
        post(route('master-data.attribute-types.scope-of-changes.store'), {
            onSuccess: (page) => {
                setData('name', '');
                setShow(false);
                loadTableData();
    ...
    return (
        <>
            <AppLayout>
                <Breadcrumb title={t('scope_of_changes')} subtitle={`${t('master_data')} / ${t('attribute_types')} / ${t('scope_of_changes')}`} />

                <div className="container">
                    <div className="d-flex justify-content-end mb-3">
                        <Button type="button" className="btn btn-sm btn-danger" data-title={t('add_scope_of_change')} onClick={handleShow}>
                            <Icon icon="line-md:plus" className="me-2" width="20" height="20" />
                            {t('add_scope_of_change')}
                        </Button>
                    </div>
                    <div className="card">
                ...
                <Modal show={show} onHide={handleClose}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <Modal.Body>
                            <div className="mb-3">
                                <label htmlFor="name" className="form-label">{t('scope_of_change_name')}</label>
                                <TextInput name="name"
                                    className="form-control"
                                    autoComplete="off"
                                    onChange={(e) => setData('name', e.target.value)}
                                    placeholder={t('enter_attribute', { 'attribute': t('scope_of_change_name') })}
                                    value={data.name}
