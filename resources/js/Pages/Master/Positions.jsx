...
export default function Positions() {
    ...
    const handleEdit = (id) => {
        setEditId(id);
        setModalTitle(t('edit_position'));
        setShow(true);
        clearErrors();
        reset();
        const position = positionData.find(pos => pos.id === id);
        if (position) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (editId) {
            return put(route('master-data.positions.update', editId), {
                onSuccess: (page) => {
                    setData('name', '');
                    setEditId(null);
                    setShow(false);
    ...
    return (
        <>
            <AppLayout>
                <Breadcrumb title={t('positions')} subtitle={`${t('master_data')} / ${t('positions')}`} />

                <div className="container">
                    <div className="d-flex justify-content-end mb-3">
                        <Button type="button" className="btn btn-sm btn-danger" data-title={t('add_new_position')} onClick={handleShow}>
                ...
                <Modal show={show} onHide={handleClose}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <Modal.Body>
                            <div className="mb-3">
                                <label htmlFor="name" className="form-label">{t('position_name')}</label>
                                <TextInput name="name"
                                    className="form-control"
                                    autoComplete="off"
                                    onChange={(e) => setData('name', e.target.value)}
                                    placeholder={t('enter_attribute', { 'attribute': t('position_name') })}
                                    value={data.name}
