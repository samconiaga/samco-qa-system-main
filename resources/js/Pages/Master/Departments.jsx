...
export default function Departments() {
    ...
    const loadDepartments = () => {
        setIsLoading(true);
        axios.get(route('datatable.departments'), {
            params: {
                page: currentPage,
    ...
    const handleDelete = (id) => {
        confirmAlert(t('are_you_sure'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.departments.destroy', id), {
                onSuccess: (page) => {
    ...
    const handleEdit = (id) => {
        setEditId(id);
        setModalTitle(t('edit_department'));
        setShow(true);
        clearErrors();
        reset();
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (editId) {
            return put(route('master-data.departments.update', editId), {
                onSuccess: (page) => {
                    setData('name', '');
                    setEditId(null);
                    setShow(false);
        ...
        post(route('master-data.departments.store'), {
            onSuccess: (page) => {
                setData('name', '');
                setShow(false);
                loadDepartments();
    ...
    return (
        <>
            <AppLayout>
                <Breadcrumb title={t('departments')} subtitle={`${t('master_data')} / ${t('departments')}`} />

                <div className="container">
                    <div className="d-flex justify-content-end mb-3">
                        <Button type="button" className="btn btn-sm btn-danger" data-title={t('add_new_department')} onClick={handleShow}>
                                ...
                                <div className="col-12">
                                    <DataTable
                                        ...
                                        noDataComponent={isLoading ? (
                                            <Loading />
                                        ) : search && departmentData.length === 0 ? (
                ...
                <Modal show={show} onHide={handleClose}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <Modal.Body>
                            <div className="mb-3">
                                <label htmlFor="name" className="form-label">{t('department_name')}</label>
                                <TextInput name="name"
                                    className="form-control"
                                    autoComplete="off"
                                    onChange={(e) => setData('name', e.target.value)}
                                    placeholder={t('enter_attribute', { 'attribute': t('department_name') })}
                                    value={data.name}
