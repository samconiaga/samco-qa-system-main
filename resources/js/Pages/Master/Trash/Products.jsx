...
export default function Products() {
    ...
    const loadTableData = () => {
        setIsLoading(true);
        axios.get(route('datatable.products.trash'), {
            params: {
                page: currentPage,
    ...
    const handleDelete = () => {
        confirmAlert(t('are_you_sure'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.products.delete'), {
                onSuccess: (page) => {
                    setData({ product_code: [] });
    ...
    const handleRestore = () => {
        confirmAlert(t('are_you_sure'), t('restore_description'), 'warning', () => {
            post(route('master-data.products.restore'), {
                onSuccess: (page) => {
                    setData({ product_code: [] });
                    const error = page.props?.flash?.error;
                    const success = page.props?.flash?.success;
    ...
    const handleSelectedRowsChange = (state) => {
        setData({ product_code: state.selectedRows.map(row => row.product_code) });
    }
    return (
        <>
            <AppLayout>
                ...
                <div className="container">
                    <div className="d-flex justify-content-end mb-3">
                        <Button type="button" className="btn btn-sm btn-success me-3" data-title={t('add_new_product')} onClick={handleRestore} disabled={processing || data.product_code.length === 0}>
                            <Icon icon="line-md:backup-restore" className="me-2" width="20" height="20" />
                            {t('restore')}
                        </Button>
                        <Button type="button" className="btn btn-sm btn-danger me-3" data-title={t('add_new_product')} onClick={handleDelete} disabled={processing || data.product_code.length === 0}>
                            <Icon icon="line-md:document-delete" className="me-2" width="20" height="20" />
                            {t('delete')}
                        </Button>
                        <Link href={route('master-data.products.index')} aria-label={t('back')} className="btn btn-sm btn-secondary" >
